...
class MessageHandler {
  ...
  Future<void> process(pb.Event event) async {
    AppLogger.d('获取消息process delivery=${event.delivery} seq=${event.seq}');

    // 1. 处理会话列表同步（优先处理！）
    if (event.type == WSEventType.conversationListSync) {
      AppLogger.d('处理会话列表同步');
    ...
    if (!chatMessageTypes.contains(event.type)) {
      AppLogger.d('非聊天消息，忽略处理: type=${event.type}');
  ...
  Future<void> _handleConversationListSync(pb.Event event) async {
    ...
    try {
      ...
      AppLogger.d("会话列表extra.data");
      AppLogger.d(data);

      final rawList = data['conversations'];
      if (rawList == null || rawList is! List) {
        AppLogger.d('会话列表同步格式错误：缺少 conversations 字段或不是 List');
        return;
      }
      ...
      AppLogger.d('收到会话列表同步，共 ${convList.length} 个会话');

      if (convList.isNotEmpty) {
        final repo = MessageRepository(Global.db!);
        await repo.syncConversationList(convList);
      } else {
        AppLogger.d('会话列表为空，无需同步到本地数据库');
      }
    } catch (e) {
      AppLogger.d('解析 conversation_list_sync 失败: $e');
    }
  }
}