...
class UserLikedPostsTab extends StatefulWidget {
  ...
  const UserLikedPostsTab({super.key, required this.userId, this.userInfo});
...
class _UserLikedPostsTabState extends State<UserLikedPostsTab> {
  ...
  Future<void> _load() async {
    ...
    try {
      final resp = await _api.getUserLikedPosts(userId: widget.userId, cursor: 0, limit: 20);
  ...
  Widget build(BuildContext context) {
    ...
    if (_posts.isEmpty) {
      return const Center(child: Text('暂无点赞'));
    }
